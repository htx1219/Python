...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # ADD CODE HERE
        noise_motion = [motion[0]+ random.gauss(0.0, self.steering_noise),
                        motion[1]+ random.gauss(0.0, self.distance_noise)]
        b = noise_motion[1]/self.length*tan(noise_motion[0])
        if abs(b) < 0.001:
            result = robot(self.length)
            result.set(self.x + noise_motion[1]*cos(self.orientation),
                       self.y + noise_motion[1]*sin(self.orientation),
    ...
    def sense(self, noise_on = 1): #do not change the name of this function
        Z = []
        for i in range(len(landmarks)):
            mark_o = atan2(landmarks[i][0]-self.y, landmarks[i][1]-self.x)
            new_o = mark_o - self.orientation
            if noise_on != 0:
                new_o += random.gauss(0.0, self.bearing_noise)
            Z.append(new_o % (2*pi))
        return Z 
    # --------
    # sense: 

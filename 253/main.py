...
def get_version(address):
    ...
    if address in all_v:
        ...
        if wiki:
            all_v[address] = wiki.version+1
        else:
            all_v[address] = 1
    return all_v[address]
...
class WikiPage(Handler):
    def get(self, address):
        ...
        if not v:
            ...
            if not wiki:
                if address == '/':
                    a = Wiki(address=address, content="<h1>Welcome to my Wiki<h1>", version = get_version(address))
                    a.put()
                    self.redirect("/wiki/")
                else:
                    self.redirect("/wiki/_edit%s" % address)
        else:
            wiki = Wiki.gql("WHERE address = :1 and version = :2", address, int(v)).get()
        self.render("wikipage.html", address=address, wiki=wiki)

class WikiHistory(Handler):
    def get(self, address):
        all_wiki = Wiki.gql("WHERE address = :1 ORDER BY created DESC", address)
        self.render("wikihistory.html", address = address, all_wiki=all_wiki)
...
class EditPage(Handler):
    ...
    def post(self, address):
        ...
        if content:
            a = Wiki(address=address, content=content, version=get_version(address))
            a.put()
...
class WikiLogout(Logout):
    def get(self):
        ...
        if address:
            self.get_to_des('/wiki%s' % address)
        else:
